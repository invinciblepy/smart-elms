...
OUT = Path(r"C:\Users\Hassan's\Downloads\lms\Smart_ELMS_Final_Report_BODY.docx")
FINAL = Path(r"C:\Users\Hassan's\Downloads\lms\Smart_ELMS_Group34_Final_Report.docx")
COVER = Path(r"C:\Users\Hassan's\Downloads\lms\_filled_cover.docx")
DECL = Path(r"C:\Users\Hassan's\Downloads\lms\_filled_declaration.docx")
ERD = Path(r"C:\Users\Hassan's\Downloads\lms\ERD diagram.png")
ASSETS = Path(r"C:\Users\Hassan's\Downloads\lms\_report_assets")
SCR = ASSETS / "screens"
DIA = ASSETS / "diagrams"
CHARTS = Path(r"C:\Users\Hassan's\Downloads\lms\smart-elms\instance\charts")
AIART = Path(r"C:\Users\Hassan's\Downloads\lms\smart-elms\ai\artefacts")
...
def add_fig(doc, path, caption, width_cm=15.4):
    path = Path(path)
    if not path.exists():
        para = doc.add_paragraph()
        para.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = para.add_run("[ Figure missing: " + path.name + " ]")
        set_run_font(run, size=10, italic=True)
        cap(doc, caption)
        return
    from PIL import Image
    ...
    w_px, h_px = im.size
    w = Cm(width_cm)
    h = Cm(width_cm * (h_px / float(w_px)))
    max_h = Cm(18.5)
    if h > max_h:
        h = max_h
        w = Cm(18.5 * (w_px / float(h_px)))
...
def build():
    ...
    hr = header.add_run("Smart ELMS  Group 34  Final report")
    ...
    h(doc, "Contents", 1)
    p(doc,
      "Word will refresh the table of contents from the heading styles when you open this file and press F9, or use References, Update Table. The official cover sheet and student declaration sit in front of this page in the assembled submission file.",
      first=False)
    para = doc.add_paragraph()
    run = para.add_run()
    fld1 = OxmlElement("w:fldChar"); fld1.set(qn("w:fldCharType"), "begin")
    ...
    figures = [
        "Figure 3.1 Layered architecture of Smart ELMS",
        "Figure 3.2 Use cases for student and instructor roles",
        "Figure 3.3 Entity relationship model normalised to third normal form",
        "Figure 3.4 Student teaching week",
        "Figure 3.5 Instructor teaching week",
        "Figure 3.6 Model training and serving",
        "Figure 3.7 Login and prediction sequence",
        "Figure 4.1 Login screen",
        "Figure 4.2 Account registration",
        "Figure 4.3 Student dashboard",
        "Figure 4.4 Course modules",
        "Figure 4.5 Assignment submission",
        "Figure 4.6 Instructor dashboard",
        "Figure 4.7 Instructor view of one student",
        "Figure 4.8 Instructor reports",
        "Figure 6.1 Assignment completion chart",
        "Figure 6.2 Login heatmap",
        "Figure 6.3 Score distribution",
        "Figure 6.4 Logistic regression coefficients",
        "Figure 6.5 Decision tree artefact",
    ]
    for item in figures:
        para = doc.add_paragraph()
    ...
    para = doc.add_paragraph()
    run = para.add_run("Table 5.1 Held out test metrics for the two candidate models")
    set_run_font(run, size=11)
    ...
    p(doc,
      "Misiejuk et al. (2025) map a newer pressure onto the same gap. Generative tools are entering learning analytics writing quickly, while most campus teaching still needs a register, a deadline and a mark. Smart ELMS stays on that smaller problem on purpose. A Decision Tree and a Logistic Regression model can be drawn in a viva. A large generative model cannot, not on this ethics basis.")
    ...
    p(doc,
      "The research design is therefore a single Flask application, a normalised SQLite schema shown in Figure 3.3, a vanilla web client, a charting service and a two model prediction experiment on synthetic data. Each choice is traceable to a source in Chapter 2 and to an objective in the four approved specifications. The next chapter describes what was built against this plan.")

    # CHAPTER 4 IMPLEMENTATION
    h(doc, "4  Implementation", 1)
    p(doc,
      "This chapter describes how the design in Chapter 3 became a running application. It is written so that a marker can see who did what without hunting. The four subsections follow the approved split of labour. Figures 4.1 to 4.8 are live screenshots of the finished artefact.")
    ...
    p(doc,
      "The student dashboard loads /api/analytics/dashboard/student and paints four stat cards, an optional support banner and a course grid. Progress bars use green, blue, orange or red according to the status label. Course pages list modules with a completed or not started state. The assignment submit page has a comment box and a dashed drop zone that accepts PDF, DOCX or TXT up to ten megabytes. After a successful submit the student sees a confirmation state and can still replace the file if the deadline has not passed. Quizzes render one card per question with options A to D and post the answers as a map of question id to letter. Figure 4.3 shows the student dashboard. Figure 4.4 shows a course module list. Figure 4.5 shows the submit form.")
    add_fig(doc, SCR / "04_student_dashboard.png", "Figure 4.3 Student dashboard")
    add_fig(doc, SCR / "06_student_course.png", "Figure 4.4 Course modules")
    add_fig(doc, SCR / "07_student_submit.png", "Figure 4.5 Assignment submission")
    p(doc,
    ...
    p(doc,
      "Support is a real page. It explains that the banner is a model output on synthetic patterns and that academic skills, the instructor and wellbeing services are the human next steps (Attewell, 2025). Remember me lengthens the stored token. The forgotten password page tells the demonstration user to contact the seeded instructor.")

    h(doc, "4.3  Backend implementation (Muhammad Hashaam Khan)", 2)
    p(doc,
      "The backend is a Flask application factory with blueprints for auth, courses, assignments, quizzes, analytics, milestones, prediction and instructor views (Flask, 2025). SQLAlchemy models map one class to one table. Student and Teacher are separate, as Figure 3.3 requires. Passwords are hashed with bcrypt before insert (OWASP, 2025b). Login issues a JWT whose identity is the numeric id and whose claims carry role, name and email, with a twenty four hour life (JWT.io, 2025). Protected routes read the Authorization header. A missing, expired or forged token returns 401. A student who posts a course returns 403.")
    p(doc,
    ...
    p(doc,
      "Quiz scoring is a ratio of correct letters to questions, stored as a 0 to 100 integer. Assignment scoring is an instructor PUT inside max_score. Both writes recompute progress and refresh the prediction, so a Friday mark can change the Monday banner.")

    h(doc, "4.4  Progress tracking implementation (Aqsa Shoukat)", 2)
    p(doc,
      "The instructor dashboard loads live aggregates: total distinct students on the instructor’s courses, mean enrolment progress as a stand in for class engagement, the count of High risk labels, and assignments due within seven days. Below the cards sit deadline alerts, performance alerts and the five class charts. Each attention row shows a name, a primary course, days since last login, a High, Medium or Low pill and a link to the profile. That list is the Course Signals idea restated for a small Flask app: a colour a busy instructor can read in one glance (Cabral, Pinto and Goncalves, 2025). Figure 4.6 shows this dashboard. Figure 4.7 shows a student profile.")
    add_fig(doc, SCR / "12_instructor_dashboard.png", "Figure 4.6 Instructor dashboard")
    add_fig(doc, SCR / "20_instructor_profile.png", "Figure 4.7 Instructor view of one student")
    p(doc,
    ...
    p(doc,
      "A typical integrated path is request, row, response. Completing a module raises progress. Submitting an assignment writes a file. An instructor score of 72 appears on the student Grades page on the next load and feeds the next prediction. If the pickle file is missing, first start trains it. If the seed is skipped, the instructor dashboard is a set of zeros. Those failures are loud on purpose.")
    ...
    p(doc,
      "Automated tests use pytest and the Flask test client (pytest, 2025). The suite now contains twenty eight cases. It covers registration and login for both roles, rejection of a short password and a wrong password, an instructor creating a course, a student enrolling, a student being blocked from creating a course, assignment submit with a valid file and with a rejected extension or MIME type, quiz attempt scoring, milestone create and update, a 401 without a token, a 403 on the wrong role, deadline and performance alerts, prediction on a full feature vector, and the health route. Coverage on the application package, including the one off seed script, sat around sixty percent on the demonstration machine. Excluding that seed script the figure sits close to the seventy percent the backend specification named. Manual click paths walk the same journeys in the browser: register a fresh account, sign in as the seeded student, open Web Development, complete a module, submit Assignment 3, sit the HTML checkpoint, sign in as the instructor, grade a submission, build a one question quiz, open Alerts and Reports. Every step in that path completed without a server error on the demonstration machine.")
    p(doc,
      "Alert arithmetic is covered by live API tests. A due date two days away with no submissions returns a yellow item. An overdue assignment with no file returns a red performance row. Chart generation is checked by requesting the five named PNG routes.")
    ...
    p(doc,
      "Responsive checks were made at 320, 768, 1024 and 1440 pixels. At phone width the navy sidebar becomes a bottom bar and the four stat cards stack. At tablet width the course grid becomes one column. Chrome, Firefox and Edge rendered the same pages without a layout break on the demonstration machine. Safari should be rechecked on a Mac before the viva. A structured accessibility walkthrough confirmed skip links, labelled fields, a keyboard usable file dropzone and visible focus. Formal WAVE and Lighthouse scores still need to be captured in Chrome on the demonstration laptop and pasted into the viva pack (W3C, 2025).")
    p(doc,
      "Firefox treated the drop zone the same as Chrome. Edge required no prefix on Fetch. The group did not own a full device lab. It did own the DevTools widths the frontend specification named.")
    ...
    p(doc,
      "This chapter reports what a marker will see when the finished application is opened, and what the models produced. The screenshots in Chapter 4 and the chart files below are taken from the running demonstration. The numbers come from the seeded database and from the held out synthetic test set.")
    ...
    p(doc,
      "The seeded instructor sees forty eight students, mean engagement of 57 percent, 19 High risk labels and one assignment due in the coming week. The attention list is ordered so that High labels and long absences rise. Opening Student A or Student C, who were seeded as less active, shows a High or Medium pill, days since last login in the high single digits, and the six model features that produced the label. Refresh prediction writes a new row from the current database facts.")
    p(doc,
    ...
    add_fig(doc, CHARTS / "completions.png", "Figure 6.1 Assignment completion chart")
    add_fig(doc, CHARTS / "heatmap.png", "Figure 6.2 Login heatmap")
    add_fig(doc, CHARTS / "histogram.png", "Figure 6.3 Score distribution")
    add_fig(doc, AIART / "coefficients.png", "Figure 6.4 Logistic regression coefficients")
    add_fig(doc, AIART / "tree.png", "Figure 6.5 Decision tree artefact")
    ...
    p(doc,
      "The viva should be four people pointing at one folder. Hafeez can show a finished teaching week in the browser. Khan can show that the same week is authenticated and stored. Shoukat can show that a teacher can see it without writing a query. Mehmood can show a probability a teacher can disagree with.")
    ...
    h(doc, "Appendix E Repository link", 1)
    p(doc,
      "The computing artefact must be uploaded to a public repository. Paste the live GitHub or OneDrive URL in the line below before the Turnitin upload.",
      first=False)
    p(doc,
      "[GROUP TO PASTE GITHUB OR ONEDRIVE LINK]",
      first=False)
    p(doc,
      "Until that URL is pasted, a marker can run the local copy. From the smart-elms folder: create a virtual environment, install requirements.txt, then run python app.py. Open http://127.0.0.1:5000. Demonstration accounts are student@university.edu and instructor@university.edu. The shared password is Password123!",
      first=False)

    h(doc, "Appendix F Project specifications", 1)
    p(doc,
      "Four approved MSc specifications sit with this report in the group folder. They are the source of the feature list.",
      first=False)
    p(doc,
      "Mohammad Hafeez, B01827888, MSc IT with Web Development: responsive login and registration, student dashboard with progress and a non threatening support banner, course and module pages, assignment submit with confirmation, WCAG minded markup, Fetch integration, and a separate instructor view.",
      first=False)
    p(doc,
      "Muhammad Hashaam Khan, B01825963, MSc IT with Web Development: a normalised schema with separate Student and Teacher tables, Flask REST authentication with bcrypt and JWT, course and lesson CRUD, file upload and download, quiz results, analytics endpoints, OWASP minded validation, and pytest evidence.",
      first=False)
    p(doc,
      "Aqsa Shoukat, B01829432, MSc IT with Project Management: instructor dashboard summaries, milestone management with deadlines, five matplotlib and seaborn chart types, yellow deadline alerts and disengagement flags, per student profiles, and live API integration.",
      first=False)
    p(doc,
      "Malik Rashid Mehmood, B01811454, MSc IT with Project Management: a 500 plus row synthetic dataset, exploratory analysis, an 80 to 20 stratified split inside a scikit-learn Pipeline, Decision Tree and Logistic Regression with GridSearchCV, written model selection, POST /api/predict, and end to end display of the risk label.",
      first=False)
    ...
    # Shared body is everything before Appendix A
    body_text = []
    hit_app = False
    refs_hit = False
    for para in doc.paragraphs:
        ...
        if t.startswith("Appendix A"):
            hit_app = True
        if (not refs_hit) and t and not t.startswith("Abstract") and not t.startswith("Contents") and not t.startswith("List of"):
    ...
    for para in doc.paragraphs:
        t = para.text.strip()
        if t.startswith("1 Introduction") or t.startswith("1  Introduction"):
            collecting = True
        if t.startswith("References"):
            collecting = False
        if collecting and t and not t.startswith("Figure ") and not t.startswith("Table "):
            main_paras.append(t)
    main_words = len(" ".join(main_paras).split())
    print("Wrote", OUT)
    print("Approximate total words including front matter, refs and appendices:", words)
    print("Approximate shared chapter words excluding refs, figures and appendices:", main_words)
    return main_words


def merge_submission():
    """Attach official cover and declaration in front of the body."""
    try:
        from docxcompose.composer import Composer
        from docx import Document as D
        master = D(str(COVER))
        composer = Composer(master)
        composer.append(D(str(DECL)))
        composer.append(D(str(OUT)))
        composer.save(str(FINAL))
        print("Assembled", FINAL)
        return True
    except Exception as exc:
        print("docxcompose merge failed:", exc)
    try:
        import win32com.client  # type: ignore
        word = win32com.client.Dispatch("Word.Application")
        word.Visible = False
        doc = word.Documents.Open(str(COVER))
        rng = doc.Range(doc.Content.End - 1, doc.Content.End - 1)
        rng.InsertBreak(7)
        rng.InsertFile(str(DECL))
        rng = doc.Range(doc.Content.End - 1, doc.Content.End - 1)
        rng.InsertBreak(7)
        rng.InsertFile(str(OUT))
        doc.SaveAs(str(FINAL))
        doc.Close(False)
        word.Quit()
        print("Assembled via Word", FINAL)
        return True
    except Exception as exc:
        print("Word merge failed:", exc)
        print("Body is at", OUT)
        print("Cover is at", COVER)
        print("Declaration is at", DECL)
        return False
